...
} from "@/components/utils/bible-parse";
...
type Item = {
  ...
  title: string;
  content: string;
  info?: { book: string; chapter: number; verse?: number; verses?: number[] };
};
    ...
    async (info: {
      ...
      try {
        if (info.verse && !info.verses) {
          ...
          for (const item of data.content as {
            verse: number;
            text: string;
          }[]) {
        ...
        return {
          title: formatReferenceTitle(info),
    ...
    const performCollection = async () => {
      ...
      editor.state.doc.descendants((node, pos) => {
        if (node.isText) {
          ...
          if (
            linkMark &&
            String(linkMark.attrs.href || "").startsWith("#bible")
          ) {
              ...
              <AnimatePresence mode="popLayout" initial={false}>
                {items.length === 0 ? (
                  ...
                  items.map((it, idx) => (
                    ...
                    >
                      <div className="mb-2 font-medium text-indigo-600 dark:text-indigo-400">
                        {it.title}
                      </div>
              ...
              <AnimatePresence mode="popLayout" initial={false}>
                {selectedItems.length === 0 ? (
                  <motion.div
                    ...
                    className="text-center text-muted-foreground text-sm py-10"
                  >
                    Selecione uma referência para visualizar.
                  </motion.div>
                ) : (
                  selectedItems.map((it, idx) => (
                    <motion.div
                      ...
                      onClick={() => {
                        ...
                        const v =
                          info.verse ??
                          (info.verses && info.verses.length > 0
                            ? info.verses[0]
                            : undefined);
