...
import { Loader2, Cloud, Upload, Edit, Trash2, ArrowLeft, Save, Download } from "lucide-react";
...
export default function CloudBackupPage() {
  ...
  // Hooks
  const { loadFile, notes, updateNote, generateUpdatedBlob, isLoading: isEditorLoading, hasLoaded } = useJwlEditor();
  ...
  const handleExportLocal = async () => {
    ...
    if (blob) {
      saveAs(blob, `${activeBackup?.name || "backup"}_editado.jwlibrary`);
      toast.success("Arquivo .JWLIBRARY exportado");
    } else {
      ...
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="secondary" onClick={handleExportLocal}>
              <Download className="mr-2 h-4 w-4"/> Exportar .JWLIBRARY
            </Button>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Editando: {activeBackup.name}</CardTitle>
    ...
    <div className="container mx-auto p-6 space-y-8">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
           {/* Botão de Upload Disfarçado */}
           <label htmlFor="upload-backup">
             <div className="flex items-center justify-center px-4 py-2 bg-black text-white rounded-md cursor-pointer hover:bg-gray-800 transition">
               <Upload className="mr-2 h-4 w-4" /> Importar Novo
             </div>
           </label>
           <Input 
             id="upload-backup" 
      ...
      {loadingList ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {backups.map((bkp) => (
            <Card key={bkp.id} className="hover:shadow-lg transition cursor-pointer border-l-4 border-l-blue-500">
              <CardHeader>
                <CardTitle className="truncate">{bkp.name}</CardTitle>
                <CardDescription>
                  Atualizado em: {new Date(bkp.updatedAt).toLocaleDateString()}
                </CardDescription>
              </CardHeader>
              <CardFooter className="flex justify-between">
                ...
                <Button variant="ghost" size="icon" className="text-red-500 hover:text-red-700 hover:bg-red-50" onClick={() => deleteBackup(bkp.id, bkp.storagePath)}>
          ...
          {backups.length === 0 && (
            <div className="col-span-3 text-center p-10 border-2 border-dashed rounded-lg text-gray-400">
              Nenhum backup encontrado. Importe seu primeiro arquivo .jwlibrary.
            </div>
          )}
        </div>
      )}
